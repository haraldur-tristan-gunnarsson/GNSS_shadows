...
mesh init_model(const char *const filepath, const GLuint position_attribute, const unsigned int win_x, const unsigned int win_y, float xoffset, float yoffset, const int useoffset){
	...
	for(unsigned int linecount = 0; fgets(line, sizeof line, obj_file); ++linecount){//Get the number of vertices in the file. TODO: error message gives incorrect line number if there are any preceding lines longer than 'sizeof line'.
		float x,y,z;
	...
	rewind(obj_file);
	for(unsigned int linecount = 0; fgets(line, sizeof line, obj_file); ++linecount){//Get the number of faces in the file. TODO: error message gives incorrect line number if there are any preceding lines longer than 'sizeof line'.
		if('f' == line[0]){
	...
	rewind(obj_file);
	for(unsigned int linecount = 0; fgets(line, sizeof line, obj_file); ++linecount){//Get the vertices from the file.
		...
		if(3 == num_got){
			vertex_array[num_added_vertices * 3 + 2] *= -1;//Flip Z-axis value.
	...
	rewind(obj_file);
	for(unsigned int linecount = 0; fgets(line, sizeof line, obj_file); ++linecount){//Get the faces from the file.
		if('f' == line[0]){
			unsigned int num_vert_indices = 0;
			int vert_indices[4];
			for(char *marker = line + 1; '\n' != *marker && '\0' != *marker;){
			...
			num_added_faces++;
			if(4 == num_vert_indices){//Handles quads as two triangles:
				element_array[num_added_faces*3 + 0] = vert_indices[0] -1;//Wavefront .obj files index vertices from 1.
...
error:
	...
	free(element_array); element_array = NULL;
	error_exit();
	(void)win_y;//Prevent compiler warning for unused parameter.
	mesh error_mesh;//Here just to avoid compiler complaints...
	return error_mesh;//...
}
